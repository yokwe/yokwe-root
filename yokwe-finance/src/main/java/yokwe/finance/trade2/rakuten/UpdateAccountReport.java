...
public class UpdateAccountReport {
	...
	private static List<AccountReportUSD> toAccountReportUSD(List<Transaction> transactionList) {
		...
		for(var entry: map.entrySet()) {
			...
			for(;;) {
				...
				lastDate = lastDate.plusDays(1);
				if (!lastDate.isBefore(date)) break;
					ret.add(FunctionUSD.reportAsOf(context, lastDate));
			}
			lastDate = date;
			
			for(var e: mapList) {
				ret.add(FunctionUSD.toAccountReportUSD(context, e));
			}
		}
	...
	private static class FunctionUSD {
		private static final Map<Transaction.Type, Map<Transaction.Asset, BiFunction<Context, Transaction, AccountReportUSD>>> map = Map.ofEntries(
			Map.entry(Transaction.Type.DEPOSIT_TRANSFER,
				Map.ofEntries(
					Map.entry(Transaction.Asset.CASH, new DEPOSTI_CASH())
				)
		...
		private static AccountReportUSD reportAsOf(Context context, LocalDate date) {
			...
			ret.stockValue     = toDollar(context.portfolio.valueAsOf(date));
			ret.stockCost      = toDollar(context.stockCost);
			ret.unrealizedGain = (ret.stockValue.signum() == -1) ? BigDecimal.ZERO : ret.stockValue.subtract(ret.stockCost);
		...
		private static class BUY implements BiFunction<Context, Transaction, AccountReportUSD> {
			...
			public AccountReportUSD apply(Context context, Transaction transaction) {
				...
				var stockCost  = context.stockCost;
				...
				ret.stockCost      = toDollar(stockCost);
				ret.unrealizedGain = (stockValue < 0) ? BigDecimal.ZERO : toDollar(stockValue - stockCost);
				ret.realizedGain   = toDollar(context.realizedGain);
				ret.dividend       = BigDecimal.ZERO;
		...
		private static class SELL implements BiFunction<Context, Transaction, AccountReportUSD> {
			...
			public AccountReportUSD apply(Context context, Transaction transaction) {
				...
				ret.stockValue     = toDollar(stockValue);
				ret.stockCost      = toDollar(stockCost);
				ret.unrealizedGain = (stockValue < 0) ? BigDecimal.ZERO : new BigDecimal(stockValue - stockCost);
				ret.realizedGain   = toDollar(context.realizedGain);
				ret.dividend       = BigDecimal.ZERO;
				ret.buy            = BigDecimal.ZERO;
