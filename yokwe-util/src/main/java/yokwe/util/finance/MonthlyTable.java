...
public final class MonthlyTable {
	private static final org.slf4j.Logger logger = yokwe.util.LoggerUtil.getLogger();
	
	public static MonthlyTable[] monthlyTableArray(String isinCode, DailyValue[] rawPriceArray, DailyValue[] divArray, int nMonth) {
		...
		BigDecimal[] dividendArray = new BigDecimal[rawPriceArray.length];
		{
			...
			for(int i = 0; i < rawPriceArray.length; i++) {
				...
				var div   = map.getOrDefault(date, BigDecimal.ZERO);
				
				priceArray[i]    = reinvestedPrice.apply(price, div);;
				dividendArray[i] = div;
			}
		}
		BigDecimal[] dailyReturnArray = BigDecimalArrays.toSimpleReturn(priceArray);
		...
		List<MonthlyTable> list = new ArrayList<>();
		for(int i = 0; i < stopIndexPlusOneArray.length - 2; i++) {
			...
			list.add(new MonthlyTable(isinCode, startIndex, stopIndexPlusOne, dateArray, priceArray, dailyReturnArray, dividendArray));
			if (list.size() == nMonth) break;
		}

		return list.toArray(MonthlyTable[]::new);
	}
	...
	private MonthlyTable(String isinCode_, int startIndex_, int stopIndexPlusOne_, 
		...
		dividend   = BigDecimalUtil.sum(dividendArray, startIndex, stopIndexPlusOne, o -> o);
	}
