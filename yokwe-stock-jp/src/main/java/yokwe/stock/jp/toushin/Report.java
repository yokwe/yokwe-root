...
public class Report implements Comparable<Report> {
	...
	public static final String getPath(String name) {
		return Toushin.getPath(String.format("%s/%s", PREFIX, name));
	}
	
	public static void save(List<Fund> list, String name) {
		Collections.sort(list);
		CSVUtil.write(Fund.class).file(getPath(name), list);
	}
	public static List<Fund> load(String name) {
		return CSVUtil.read(Fund.class).file(getPath(name));
	}
